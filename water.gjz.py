...
# 用fiddler抓包对应headers里的内容
host_ = "szzdjc.cnemc.cn:8070"
url = "https://{}/GJZ/Ajax/Publish.ashx".format(host_)

# AreaID=&爬取全国流域数据
payload = "AreaID=&RiverID=&MNName=&PageIndex=-1&PageSize=60&action=getRealDatas"
headers = {
    'Host': host_,
    ...
    'Origin': 'https://szzdjc.cnemc.cn:8070'.format(host_),
    'Referer': 'https://szzdjc.cnemc.cn:8070/GJZ/Business/Publish/RealDatas.html'.format(host_),
...
sql_table = "my_t_water_monitor_auto"
        ...
        for item in tbody:
            ...
            for sub in item:
                if sub is not None:  # 主要判断监测时间是否为空
                    if 'span' in sub:
                        ret = re.findall(
                            r'<span data-toggle=\'tooltip\' data-placement=\'right\' title=\'(.*?)\'>(.*?)</span>', sub,
                            re.S)
...
if __name__ == '__main__':
    ...
    # count = []
    thead_sub, tbody_sub = analyse(get())
    insertSQL(tbody_sub)
    print("数据成功写入数据库")
